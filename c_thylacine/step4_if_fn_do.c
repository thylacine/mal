...
static
MalData *
eval_closure(MalClosure *mc, MalData **values, size_t values_len)
{
	MalData *md;
	MalEnv *env;
	...
	env = MalEnv_new(mc->env, mc->params, values, values_len);

	md = mal_eval(mc->body, env);
...
mal_eval(MalData *ast, MalEnv *env)
{
	...
	if (ast && ast->type == MAL_LIST) {
		...
		case MAL_CLOSURE:
			return eval_closure((MalClosure *)first, ((MalMulti *)md)->multi + 1, ((MalMulti *)md)->len - 1);
