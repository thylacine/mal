	...
	if (ast && ast->type == MAL_LIST) {
		...
		if (first->type == MAL_SYMBOL) {
			if (strcmp(((MalSymbol *)first)->symbol, "def!") == 0) {
				...
				goto tail_call;
			} else if (strcmp(((MalSymbol *)first)->symbol, "try*") == 0) {
				MalData *expr, *catch, *catchsym;

				if (((MalMulti *)ast)->len != 3) {
					LOG_ERROR("%s expected %zu argument%s, got %zu", "try*", 2, "s", ((MalMulti *)ast)->len - 1);
					E_THROW(E_PARSE_FAILURE, NULL);
				}

				expr = ((MalMulti *)ast)->multi[1];
				catch = ((MalMulti *)ast)->multi[2];

				E_START
				case E_TRY:
					md = mal_eval(expr, env);
					break;
				...
				case E_UNDEFINED_SYMBOL:
					if (catch->type == MAL_LIST
					&&  ((MalMulti *)catch)->len > 0
					&&  (catchsym = ((MalMulti *)catch)->multi[0])
					&&  catchsym->type == MAL_SYMBOL
					&&  strcmp(((MalSymbol *)catchsym)->symbol, "catch*") == 0) {
						MalEnv *cenv;
						MalData *esym;
						MalData *emsg;

						if (((MalMulti *)catch)->len != 3) {
							LOG_ERROR("%s expected %zu argument%s, got %zu", "catch*", 2, "s", ((MalMulti *)catch)->len);
							E_THROW_UP(E_PARSE_FAILURE, NULL);
							break;
						}
						esym = ((MalMulti *)catch)->multi[1];
						if (esym->type != MAL_SYMBOL) {
							LOG_ERROR("%s argument %zu must be %s", "catch*", 1, "symbol");
							E_THROW_UP(E_TYPE, esym);
							break;
						}

						cenv = MalEnv_new(env, NULL, NULL, 0);
						cenv->args = 1;

						/* exception messages */
						switch (__e) {
						case E_UNDEFINED_SYMBOL:
							emsg = MalData_new(MAL_STRING);
							strbuf_appendf((char **)&(((MalString *)emsg)->string), &(((MalString *)emsg)->alloc), &(((MalString *)emsg)->len), "'%s' not found", ((MalSymbol *)E_DATA)->symbol);
							break;

						default:
							emsg = E_DATA;
							if (emsg == NULL) {
								emsg = MalData_new(MAL_NIL);
							}
						}

						MalEnv_set(cenv, (MalSymbol *)esym, (MalData *)emsg);
						md = mal_eval(((MalMulti *)catch)->multi[2], cenv);
					} else {
						md = MalData_new(MAL_NIL);
					}
					break;
...
main(int argc, char *argv[])
{
	...
	(void)mal_eval(read_str("(def! *gensym-counter* (atom 0))"), repl_env);
	(void)mal_eval(read_str("(def! gensym (fn* [] (symbol (str \"G__\" (swap! *gensym-counter* (fn* [x] (+ 1 x)))))))"), repl_env);
	...
	(void)mal_eval(read_str("(defmacro! or (fn* (& xs) (if (empty? xs) nil (if (= 1 (count xs)) (first xs) (let* (condvar (gensym)) `(let* (~condvar ~(first xs)) (if ~condvar ~condvar (or ~@(rest xs)))))))))"), repl_env);
	...
	if (script) {
		...
		case E_MAL:
			fprintf(stderr, "throw without try\n");
			md_dumpf(stderr, E_DATA);
			exit_code = EX_DATAERR;
			break;
